...
- **The Terms Gate (Legal Lock):** 
    ...
    - Must scroll and accept `artist-terms.md` before dashboard hydration.
...
- **Admin Control:** The `commissionRate` is strictly managed by **Admins**. Common artists have read-only access to their rates to prevent unauthorized changes.
- **Settlement Process:**
...
## 🛠️ 05. TECHNICAL STACK & CONSTRAINTS
...
- **Rules:**
    ...
    - **Image Engine:** Native local file upload with Base64 encoding for instant reactivity (Profile, Studio, Inventory).
    - **Next.js 15 Stability:** All dynamic `searchParams` and `params` are correctly `awaited` as Promises.
    - **Supreme Performance:** CSS `will-change` hints and optimized Re-render loops in sensitive pages.

---

## 🚧 06. RECENT DECISIONS & "DEPRECATIONS"
